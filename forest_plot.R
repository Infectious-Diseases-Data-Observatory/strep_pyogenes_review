...
plot_bubble = ggplot(gas_mean,
  ...
  geom_point(data = gas_mean,
             size = 5, alpha = 0.65, show.legend = FALSE) +
...
layout <- c(
  ...
  patchwork::area(t = 0, l = 17, b = 30, r = 20) # right most plot starts at top of page, begins where middle plot ends (l=9, and middle plot is r=9), goes to bottom of page (b=30), and extends two units wide (r=11)
)

plot_regions + plot_bubble + plot_diffs + patchwork::plot_layout(design = layout)

ggsave("mean_mic_forest_mean.tiff", path = "figures/", width = 17.5, height = 9.5)
